...
class urlNode(object):
    class Struct(object):
        def __init__(self, url,title,status,preurl,url_name):
            self.url = url
            self.title = title
            self.status = status
            self.preurl = preurl
    ...
    def make_struct(self, url,title,status,preurl,url_name):
        return self.Struct(url,title,status,preurl,url_name)
urlNode = urlNode()
...
def GetLinkHasNetloc(firstlink,childlink):
    ...
    if (childlink[:1] == '/'):
        if ('doc' in childlink):
            ...
            # print('my_url.netloc' + my_url.netloc)
            # print('firstlink' + firstlink)
            # print('外部的resultLink : ' + resultLink)
            if(get_proxy(resultLink) != None):
                # print('有效的resultLink : ' + resultLink)
                return resultLink
            else:
                # print('无效的resultLink : ' + resultLink)
                resultLink = my_url.scheme + '://' + my_url.netloc + my_url.path + childlink
                # print('换了一种拼接方式的resultLink : ' + resultLink)
                if (get_proxy(resultLink) != None):
                    return resultLink
                # else:
    testurl = my_url.scheme + '://' + my_url.netloc + childlink
    if(get_proxy(testurl)):
        return testurl
    # 如果是绝对路径
    if (childlink[:1] == '.'):
...
def addnode(url,title,status,preurl,url_name):
    if ('招聘' in title):
        node1 = urlNode.make_struct(url, title, status, preurl,url_name)
    ...
    if (status == 1):
        ...
        if (isincludeUse):
            ...
            node2 = urlNode.make_struct(url, title, status, preurl,url_name)
            secondresult.add(node2)
    if (status == 2):
        if ('更多' in title):
            node4 = urlNode.make_struct(url, title, status, preurl,url_name)
...
class location(object):
    class Struct(object):
        def __init__(self, province,city,area):
            ...
            self.city = city
            self.area = area
    def make_struct(self, province,city,area):
        return self.Struct(province,city,area)
location = location()
