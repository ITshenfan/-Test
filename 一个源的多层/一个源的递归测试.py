...
def getThreeUtl(secondurl):
    ...
    for i in range(1,6):
        ...
        try:
            ...
            response = requests.request("GET", secondurl, headers=headers)  # 获取网页数据
            response.encoding = response.apparent_encoding  # 当获取的网页有乱码时加
            soup = BeautifulSoup(response.text, 'html.parser')
            bf = soup.find('div', class_='view TRS_UEDITOR trs_paper_default trs_web')
            # 提取二级链接的地址
    ...
    for i in range(1,6):
        ...
        try:
            ...
            if (a):
                ...
                if zhaopinresult:
                    print('级别 ：' + status + '  存在招聘信息的标题::' + a.title + '         链接== ' + url)
                if (status == '2'):
                    guolvResult = guolv(a.title, url, status)
                    if (guolvResult == ''):
                        return
                if (status == '3'):
                    print('级别 ：' + status + '                 不招聘信息的标题::' + a.title + '         链接== ' + url)

        except:
            print("服务器拒绝连接........，休息5s ，第%d次    "%i + url)
            print("ZZzzzz...")
            time.sleep(5)
            print("做了个美美的梦，睡的很好, 那我们继续吧...")
            print('=' * 40)
